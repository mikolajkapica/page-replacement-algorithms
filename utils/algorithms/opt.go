...
func Opt(pages []int, framesQuantity int) int {
	//fmt.Println("OPT_________________________________")
	var pageFaults = 0
	var currentFrame = 0
	...
	for i := 0; i < len(pages); i++ {
		...
		if !utils.Contains(frames, pages[i]) {
			pageFaults++
			currentFrame = NotUsedForLongestTimeInFuture(pages, frames, i)
...
func NotUsedForLongestTimeInFuture(pages []int, frames []int, currentPageIndex int) int {
	checkedFrames := make([]int, 0)
	// add frames that are used in future
	for i := currentPageIndex + 1; i < len(pages); i++ {
